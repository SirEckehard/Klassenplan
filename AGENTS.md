...
- ✅ Tests: 2341 unit tests (237 test files) + 9 Playwright tests (3 smoke + 2 core flow + 4 onboarding), 100% passing
...
- ✅ i18n: Bilingual support (German/English) fully implemented, DE/EN key parity 1:1 (2126 keys per language)
...
- **The workspace has no page footer.** `App.tsx` leaves it off `/generator` for the same reason as the fullscreen routes, and `HeaderAppMenu` carries what a teacher reaches for from inside a plan and no layer owns: theme and language, the update check, wiping all data (`AppSettingsItems` with `storage={false}`, shared with the footer's own gear) and the two legal pages. The backup and the saved plans are not repeated there — the class layer's toolbar carries the backup ("Backup" → export/import), the plan layer's the saved plans. FAQ, feedback, support and the changelog stay on the pages they belong to.
...
- **The toolbar has no header of its own.** Which density it shows is a property of the workspace, so the switch sits in the status bar and the state lives in `ToolRailContext` (⌘/Strg+B still works). A sidebar outside the shell — the export page's — finds no provider, keeps its own state and its own switch, and names its column widths through `widths`.
...
- **Editing a thing happens in the inspector, not in the list.** `InspectorContext` holds what is selected (students so far; tables and features follow), and `StudentInspector` groups the controls under the pedagogical family they belong to. Adding an attribute means adding it to one group there — never a new column.
- **A list row is one button.** The class list is a single card of 60px rows divided by a hairline: number, avatar (`StudentAvatar`, to look at, not to press), name and the chips for what is set. Pressing the row opens it in the inspector — which is also where the name, the photo and the one destructive action live. The checkbox for bulk edits stays outside that button, because a checkbox inside a button is not a checkbox.
