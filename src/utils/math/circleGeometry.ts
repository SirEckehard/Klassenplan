...
/**
 * Finds the best starting angle to minimize disruption for specific students
 */
export function findOptimalStartAngle(priorityStudentIds: string[]): number {
  if (priorityStudentIds.length === 0) {
    return 0; // Default start angle
  }

  // For special needs students, prefer starting them at bottom-right
  // where they're closest to typical  position
  return 315; // Bottom-right (7 o'clock position)
}

/**
 * Advanced geometry calculations for Phase 2 optimization
 */

/**
 * Calculates the optimal circle curvature for natural conversation flow
 * Returns adjusted radius values for better social dynamics
 */
export function calculateOptimalCurvature(
  studentCount: number,
  conversationIntensity: number = 0.7, // 0-1, higher = more curved for intimacy
): { horizontal: number; vertical: number; curvatureScore: number } {
  // Base calculation
  const baseRadius = (studentCount * 45) / (2 * Math.PI);

  // Conversation dynamics: smaller groups benefit from tighter curves
  const groupSizeFactor = Math.max(0.6, 1 - (studentCount - 8) / 20);
  const conversationFactor = 0.8 + conversationIntensity * 0.4;

  const horizontal = baseRadius * 1.3 * groupSizeFactor * conversationFactor;
  const vertical = baseRadius * groupSizeFactor * conversationFactor;

  // Score the curvature quality (0-1, higher is better)
  const curvatureScore = Math.min(1, groupSizeFactor * conversationFactor);

  return { horizontal, vertical, curvatureScore };
}

/**
 * Optimizes circle layout for acoustic properties
 * Considers hearing impairments and conversation dynamics
 */
export function calculateAcousticOptimization(
  ...
  const acousticScore = 1.0; // Default good score since no  positioning needed
  const conversationZones: Array<{
    center: { x: number; y: number };
    radius: number;
    studentIds: string[];
  }> = [];

  // Calculate conversation zones (groups of 3-4 adjacent students)
  const sortedByAngle = [...studentPositions].sort((a, b) => a.angle - b.angle);

  for (let i = 0; i < sortedByAngle.length; i += 3) {
    const zoneStudents = sortedByAngle.slice(i, i + 3);
    if (zoneStudents.length >= 2) {
      const centerX =
        zoneStudents.reduce((sum, s) => sum + s.x, 0) / zoneStudents.length;
      const centerY =
        zoneStudents.reduce((sum, s) => sum + s.y, 0) / zoneStudents.length;

      conversationZones.push({
        center: { x: centerX, y: centerY },
        radius: 60,
        studentIds: zoneStudents.map((s) => s.student.id),
      });
    }
  }
...
/**
 * Validates circle layout geometry for physical feasibility
 */
export function validateCircleGeometry(
  studentPositions: CircleStudentPosition[],
): {
  isValid: boolean;
  issues: string[];
  ...
  // Check minimum distances between students
  for (let i = 0; i < studentPositions.length; i++) {
    for (let j = i + 1; j < studentPositions.length; j++) {
      ...
      if (distance < 30) {
        issues.push(
          `Students ${studentPositions[i]!.student.name} and ${studentPositions[j]!.student.name} are too close (${distance.toFixed(1)}px)`,
        );
      }
    }
  }
  ...
  // Check if all students are within classroom bounds
  for (const position of studentPositions) {
    if (
      position.x < 20 ||
      position.x > 880 ||
      position.y < 20 ||
      ...
      issues.push(
        `Student ${position.student.name} is outside classroom bounds`,
      );
    }
  }
