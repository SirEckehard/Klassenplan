...
- **Font.** The Latin Instrument Sans file is preloaded (`preloadPrimaryFont` in `vite.config.ts`), and `Instrument Sans Fallback` in `src/index.css` gives Arial its metrics so the swap does not move text. Instrument Serif carries headings only and stays unpreloaded — it is `font-display: swap` behind `Instrument Serif Fallback` (Times New Roman).
