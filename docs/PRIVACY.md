...
| Behaviour and support attributes: restless, shy, distractible, front seat, high/low performer, language level, social role | Class record                              | Same                                           | In the editor; on the projector only in the teacher perspective with "Merkmale" switched on (off by default); in exports while "Bedürfnisse" is on (on by default), family by family |
...
- **Recipients of exported files.** Only the backup is encrypted. A printed plan,
  ...
  contains every attribute. The export page chooses per family which attributes
  a sheet carries, so a plan for the classroom wall can leave out behaviour and
  learning ([decision 0021](decisions/0021-seat-badges-explained.md)).
