...
| ------------------------------------------ | --------- | ---------------------------------------------------------------------------------------- | ------------------------- |
| `src/components`, `src/pages` (enforced)   | yes       | type imports, `seatingStatistics`, `criterionHighlights`, `planReasons`, `circleSummary` | type imports, `planUsage` |
...
- **`seatingStatistics`, `criterionHighlights`, `planReasons`, `circleSummary`, `planUsage`** are pure, synchronous derivations of what is on screen: the fulfilment beside each criterion, the criterion highlights on the seats, the sentences under "Warum dieser Plan", the circle's inspector and the arcs between its table neighbours, and the neighbourhood view. They have to show exactly what the algorithm scores – `isCountedUsage()` is shared by the evaluation and the scoring for precisely that reason ([ALGORITHM.md](ALGORITHM.md#confirmation)). Wrapping them in hooks would add a layer without protecting anything.
...
- **`src/components` and `src/pages`** use `@typescript-eslint/no-restricted-imports`: `@/utils/algorithm` and `@/utils/data` are blocked, including their index modules and nested paths, except for type imports and the derivations listed above.
