...
`TOAST_MESSAGES` maps stable keys (e.g. `SAVE_ERROR`) to i18n paths (e.g. `'toast:save.error'`). The toast renderer resolves the path at runtime via `i18n.t()` — any message containing a `:` namespace separator is treated as an i18n key. This prevents hard-coded strings and enforces DE/EN consistency.
...
The default messages of `handleError`/`errorHandlers` are i18n keys as well (`toast:errors.network`, `toast:errors.storage`, …); custom messages passed to the handlers should also be keys, never raw strings. The same applies to error constants in utils (e.g. `BACKUP_ERROR_MESSAGES`, `STUDENT_PHOTO_ERRORS`): utils stay language-free and return keys.

One more rule for async code: fire-and-forget promises for I/O must attach a `.catch` with logging — a bare `void somePromise()` turns storage failures into unhandled rejections.
...
errorHandlers.storageError(
  error,
  'Background migration could not be completed.',
);
```
...
- [src/hooks/wizard/**tests**/useSeatingWizard.test.ts](../src/hooks/wizard/__tests__/useSeatingWizard.test.ts) – failure paths of `saveSeatingPlan`
- [src/stores/**tests**/algorithmStore.test.ts](../src/stores/__tests__/algorithmStore.test.ts) – `setShowStatisticsBadge` with `localStorage.setItem` throwing
- [src/stores/**tests**/studentsStore.test.ts](../src/stores/__tests__/studentsStore.test.ts) – `importCsv` with an empty service result
