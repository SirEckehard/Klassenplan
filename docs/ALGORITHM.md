...
**Concept**: A "Hill Climbing" approach. The algorithm looks for swaps that *immediately* improve the total score (lower is better). It never accepts a swap that makes the arrangement worse.
...
*   If a swap *worsens* the score, it is **accepted with a probability** that depends on:
