...
Tags follow the release: `2.0.2` and `2.0` pin a version, `latest` always tracks the newest release. The easiest way to run it is via [`docker-compose.yml`](docker-compose.yml):
...
| --------------------- | ------------------------ | ------------------------------------------------------------- |
| `KLASSENPLAN_VERSION` | `latest`                 | Pins the image tag, e.g. `2.0.2`                              |
