...
- `npm run generate:sitemap` - Generate sitemap before builds
- `vitest run --reporter=verbose` - Run tests with detailed output
- `vitest run src/path/to/test.test.ts` - Run a single test file

## Core Architecture

React-based classroom seating plan generator with a multi-step wizard, layered contexts and repository-backed persistence.
...
2. **Domain contexts** split responsibilities:
   - `StudentManagementContext` – student CRUD, CSV import/export, placeholder tools
...
3. **useSeatingGenerator** orchestrates repositories, undo/redo stacks, worker-based algorithm calls and UI signals.
